...
<body>
    <div class="modal-overlay active" id="buscarModal">
        <div class="modal-content">
            ...
            <h2>Buscar Usuario <?= $tipo === 'id' ? 'por ID' : 'por Nombre' ?></h2>
            <form method="post" action="buscar.php?tipo=<?= htmlspecialchars($tipo) ?>">
                <div class="form-group">
                    <?php if ($tipo === 'id'): ?>
                        <label for="buscarId">ID</label>
                        <input type="text" id="buscarId" name="buscarId" maxlength="3" required placeholder="Ingrese el ID del usuario">
                    <?php elseif ($tipo === 'nombre'): ?>
                        <label for="buscarNombre">Nombre</label>
                        <input type="text" id="buscarNombre" name="buscarNombre" maxlength="15" required placeholder="Ingrese el nombre de usuario">
                    <?php else: ?>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($tipo === 'id' && isset($_POST['buscarId'])) {
        ...
        $stmt = $con->prepare("SELECT * FROM usuarios WHERE nomUsuario LIKE ?");
        $stmt->execute(['%' . $nombre . '%']);
        $resultados = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if ($resultados) {
            $popup = [
